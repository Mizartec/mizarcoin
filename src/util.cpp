...
const char * const BITCOIN_CONF_FILENAME = "mizarcoin.conf";
const char * const BITCOIN_PID_FILENAME = "mizarcoin.pid";
...
#else
    const char* pszModule = "mizarcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mizarcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Mizarcoin";
#else
    // Unix
    return pathRet / ".mizarcoin";
#endif
#endif
}
